...
PIDTrainer::PIDTrainer(PID* p, double threshold)
{
  ...
  increment_step[1] = 1.2;
  increment_step[2] = 1.2;
  //increment_step = 1.1;
  ...
  //if(_currState != TRAINING_INIT) {
    ...
    if(sum < _threshold) {
      std::cout << "TRAINING COMPLETE sum = " << sum  << " Threshold = " << _threshold <<
        " Best Param = " << " Kp = " << _param[0] << " Kd = " << _param[1] <<
        " Ki = " << _param[2] << " We reach the threshold after " << _number_of_calls << " iterations" << std::endl;
      _best_param = _param;
  ...
  switch(_currState) {
    ...
    case EVALUATE_ERROR_AFTER_DECREASE:
      ...
      if(error < _best_error) {
        ...
        //  _dp[_next_index] = std::numeric_limits<double>::epsilon();
      }
      _next_index = (_next_index + 1) % 3;
      //std::cout << "\tNext index will be " << _next_index << std::endl;
      _currState = INCREASE_COEFFICIENT;
    ...
    case TRAINING_COMPLETE:
      std::cout << "XXXXXXXXXXXXXXXXXXXXXX TRAINING COMPLETE we retrained the model " << _total_train
        << " times so far. Params kp " << _param[0] << " kd = " << _param[1]
        << " Ki = " << _param[2] << std::endl;
      _best_param = _param;
      _pid->Init(_param[0], _param[2], _param[1]);
      _pid->setTuned(true);
      _threshold = _threshold / _total_train;
      _total_train++;
      if(_threshold <= std::numeric_limits<double>::epsilon()) {
        std::cout << "AVOIDED THRESHOLD UNDERFLOW " << std::endl;
